...
def ec2_usage():
    ...
    logger.info(f"ec2_all_count: {EC2_ALL_COUNT}, ec2_anomaly_period_count:{EC2_ANOMALY_PERIOD_COUNT}")
...
def ebs_usage():
    ...
    logger.info(f"ebs_all_count: {EBS_ALL_COUNT}, ebs_anomaly_period_count:{EBS_ANOMALY_PERIOD_COUNT}")
...
def lambda_usage():
    ...
    logger.info(f"lambda_usage: {LAMBDA_USAGE}")
    return LAMBDA_USAGE
    ...
    for record in event["Records"]:
        ...
        if alert_subject:
            if "ec2" in alert_subject:
                ...
                logger.error(f"Failed to get service specific subject.")
